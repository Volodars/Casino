...
const flipSound = new Audio('../Music/Baraban.mp3'); // Sound for the coin flipping animation
...
// --- Coin Configuration ---
const COIN_SIDES = [
    { name: 'heads', text: '👤', color: '#FFD700', multiplier: 2 },
    { name: 'tails', text: '1', color: '#463f3f', multiplier: 2 },
...
function drawCoin(rotationAngle = 0, scaleY = 1, finalSideObj = null, currentConceptualSideIndex = null) {
    ...
    if (finalSideObj || currentConceptualSideIndex !== null) {
        ctx.save(); // Save context for text transformations
        // Reverse scale for text to prevent distortion from coin's scaleY
        ...
        let textY = 0; // Default text position is center
        if (finalSideObj && finalSideObj.name === 'edge') {
            textY = -radius * 0.5; // Offset 'EDGE' text slightly up
        }

        ctx.fillText(contentToDraw, 0, textY); // Draw text with the new Y-coordinate
...
function flipCoin() {
    ...
    if (playerBetSide === finalWinningSideName) {
        ...
        winSound.play(); // NEW: Play win sound IMMEDIATELY (as requested for easy testing)
    } else {
