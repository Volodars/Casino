...
const spinSound = new Audio('../Music/Baraban.mp3');
...
let animationFrameId = null; // NEW: Stores the requestAnimationFrame ID
...
function updateDisplay() {
    // NEW: Use the global updateBalanceDisplay function from promo.js
...
function spinReels() {
    ...
    isSpinning = true;
    // NEW: Disable all relevant buttons using a new function
    disableGameControls();
    ...
    // NEW: Generate final reels immediately after bet is placed
    const finalReels = [];
    for (let i = 0; i < reelCount; i++) {
        let column = [];
        for (let j = 0; j < symbolsPerReel; j++) {
    ...
    // NEW: Check for win and calculate winnings IMMEDIATELY
    ...
    function animate(timestamp) {
        ...
        if (elapsed < duration) {
            ...
            // NEW: No need to generate new reels here, currentReels already holds finalReels
            // for (let i = 0; i < reelCount; i++) {
            //     currentReels[i] = [];
            //     for (let j = 0; j < symbolsPerReel; j++) {
            ...
            // NEW: winMultiplier and winnings already determined at the start of spinReels()
    ...
    animationFrameId = requestAnimationFrame(animate); // NEW: Start animation loop and store its ID
}
...
if (allInButton) { // Check if the button exists on this page
    allInButton.addEventListener('click', () => {
        ...
        // NEW: localStorage.setItem here is no longer needed, balance will be saved in spinReels() on the next spin.
...
// NEW: Event listener for page visibility changes (critical for immediate balance update)
