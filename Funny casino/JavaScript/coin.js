...
const imagePaths = {
    'heads_img': '../Images/coin_head1.png', // Path to your coin heads image
    'tails_img': '../Images/coin_tail1.png'  // Path to your coin tails image
};
...
const COIN_SIDES = [
    { name: 'Boobs', imageId: 'heads_img', multiplier: 2 },
    { name: 'Booty', imageId: 'tails_img', multiplier: 2 },
    { name: 'Both', text: 'EDGE', color: '#8B4513', multiplier: 48 }
];
...
function updateDisplay() {
    ...
    if (playerBetSide === 'Boobs') betHeadsButton.classList.add('selected');
    else if (playerBetSide === 'Booty') betTailsButton.classList.add('selected');
    else if (playerBetSide === 'Both') betEdgeButton.classList.add('selected');
}
...
function drawCoin(rotationAngle = 0, scaleY = 1, finalSideObj = null, currentConceptualSideIndex = null) {
    ...
    if (sideToShow) {
        ...
        if (sideToShow.name === 'Both') {
            ...
            if (image) {
                const imgSize = radius * 2; // Image takes 100% of coin diameter not 90%
...
function flipCoin() {
    ...
    if (playerBetSide === null) {
        resultText.textContent = 'Please place a bet (Boobs, Booty or Both (rare))!';
        return;
    }
    ...
    function animate(timestamp) {
        ...
        if (progress < 1) {
            ...
            currentScaleY = (finalWinningSideName === 'Both') ? 0.05 : 1;
        }
...
betHeadsButton.addEventListener('click', () => {
    ...
    playerBetSide = 'Boobs';
    resultText.textContent = `Bet placed on Boobs! Amount: ${currentBet}`;
    updateDisplay();
});
...
betTailsButton.addEventListener('click', () => {
    if (isFlipping) { resultText.textContent = "Cannot change bet while flipping!"; return; } 
    playerBetSide = 'Booty';
    resultText.textContent = `Bet placed on Booty! Amount: ${currentBet}`;
    updateDisplay();
});
...
betEdgeButton.addEventListener('click', () => {
    if (isFlipping) { resultText.textContent = "Cannot change bet while flipping!"; return; } 
    playerBetSide = 'Both';
    resultText.textContent = `Bet placed on Both! Amount: ${currentBet}`;
    updateDisplay();
});
